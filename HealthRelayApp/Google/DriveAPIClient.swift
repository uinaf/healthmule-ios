...
actor DriveAPIClient {
    ...
    private func findTaggedItem(
        ...
        var clauses = [
            "trashed = false",
            "appProperties has { key='healthRelayKind' and value='\(escapedQueryValue(kind))' }",
        ]
        if kind != "root" {
            clauses.insert(
        ...
        if let date {
            clauses.append(
                "appProperties has { key='healthRelayDate' and value='\(escapedQueryValue(date))' }"
            )
        }
    ...
    private func createFolder(
        ...
        let payload = DriveCreateMetadata(
            ...
            parents: [parentID],
            appProperties: ["healthRelayKind": kind]
        )
        var components = URLComponents(
            url: Self.filesBaseURL,
            resolvingAgainstBaseURL: false
    ...
    private func multipartUpload(
        ...
        let boundary = "health-relay-\(UUID().uuidString)"
    ...
    private static func isUsableJSONArtifact(
        ...
        guard
            ...
            item.appProperties?["healthRelayKind"] == kind
        else {
            return false
        }
        return item.appProperties?["healthRelayDate"] == date
    }
